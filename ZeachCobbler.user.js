...
var observer = new MutationObserver(function (mutations) {
		mutations.forEach(function (mutation) {
			...
			if (newNodes !== null) { // If there are new nodes added
				if (newNodes[0].tagName == "SCRIPT" && newNodes[0].src.search('agar.io/main_out.js') > -1) {
					newNodes[0].remove();
					observer.disconnect()
				};
			}
...
document.addEventListener("DOMContentLoaded", function () {
	...
	(function (g, m) {
		...
		var cobbler = {
			...
			set isLiteBrite(val) {
				this._isLiteBrite = val;
				GM_setValue('isLiteBrite', val);
			},
			get isLiteBrite() {
				return this._isLiteBrite;
			},
			_sfxVol : GM_getValue('sfxVol', 0.5),
			set sfxVol(val) {
				this._sfxVol = val;
				GM_setValue('sfxVol', val);
			},
			get sfxVol() {
				return this._sfxVol;
			},
			...
			set drawTail(val) {
				this._drawTail = val;
				GM_setValue('drawTail', val);
			},
			get drawTail() {
				return this._drawTail;
			},
			_splitGuide : GM_getValue('splitGuide', true),
			set splitGuide(val) {
				this._splitGuide = val;
				GM_setValue('splitGuide', val);
			},
			get splitGuide() {
				return this._splitGuide;
			},
			_rainbowPellets : GM_getValue('rainbowPellets', true),
			set rainbowPellets(val) {
				this._rainbowPellets = val;
				GM_setValue('rainbowPellets', val);
			},
			get rainbowPellets() {
				return this._rainbowPellets;
			},
			_debugLevel : GM_getValue('debugLevel', 1),
			set debugLevel(val) {
				this._debugLevel = val;
				GM_setValue('debugLevel', val);
			},
			get debugLevel() {
			...
			set imgurSkins(val) {
				this._imgurSkins = val;
				GM_setValue('imgurSkins', val);
			},
			get imgurSkins() {
				return this._imgurSkins;
			},
			_amExtendedSkins : GM_getValue('amExtendedSkins', true),
			set amExtendedSkins(val) {
				this._amExtendedSkins = val;
				GM_setValue('amExtendedSkins', val);
			},
			get amExtendedSkins() {
				return this._amExtendedSkins;
			},
			_amConnectSkins : GM_getValue('amConnectSkins', true),
			set amConnectSkins(val) {
				this._amConnectSkins = val;
				GM_setValue('amConnectSkins', val);
			},
			get amConnectSkins() {
				return this._amConnectSkins;
			},
			_namesUnderBlobs : GM_getValue('namesUnderBlobs', false),
			set namesUnderBlobs(val) {
				this._namesUnderBlobs = val;
				GM_setValue('namesUnderBlobs', val);
			},
			get namesUnderBlobs() {
				return this._namesUnderBlobs;
			},
			_grazerHybridSwitch : GM_getValue('grazerHybridSwitch', false),
			set grazerHybridSwitch(val) {
				this._grazerHybridSwitch = val;
				GM_setValue('grazerHybridSwitch', val);
			},
			get grazerHybridSwitch() {
				return this._grazerHybridSwitch;
			},
			_grazerHybridSwitchMass : GM_getValue('grazerHybridSwitchMass', 300),
			set grazerHybridSwitchMass(val) {
				this._grazerHybridSwitchMass = val;
				GM_setValue('grazerHybridSwitchMass', val);
			},
			get grazerHybridSwitchMass() {
				return this._grazerHybridSwitchMass;
			},
			_gridLines : GM_getValue('gridLines', true),
			set gridLines(val) {
				this._gridLines = val;
				GM_setValue('gridLines', val);
			},
			get gridLines() {
			...
			set autoRespawn(val) {
				this._autoRespawn = val;
				GM_setValue('autoRespawn', val);
			},
			get autoRespawn() {
				return this._autoRespawn;
			},
			//        "respawnWithGrazer" : false,
			_visualizeGrazing : GM_getValue('visualizeGrazing', true),
			set visualizeGrazing(val) {
				this._visualizeGrazing = val;
				GM_setValue('visualizeGrazing', val);
			},
			get visualizeGrazing() {
				return this._visualizeGrazing;
			},
			"displayMiniMap" : true,
			"clickToShoot" : false,
		};
		...
		var zeach = {
			...
			get mapWidth() {
				return ~~(Math.abs(zeach.mapLeft) + zeach.mapRight)
			},
			get mapHeight() {
				return ~~(Math.abs(zeach.mapTop) + zeach.mapBottom)
			},
		};
		...
		function augmentBlobArray(blobArray) {
			...
			ghostBlobs = _.filter(ghostBlobs, function (element) {
					...
					(
						...
						(Math.abs(viewport.y - element.ny) > (viewport.dy + element.nSize) * 0.9)); // outside of firmly visible area, otherwise there's no need to remember it
				});
		...
		function findFoodToEat_old(cell, blobArray) {
			...
			edibles = edibles.sort(function (x, y) {
			...
			var target = densityResults.sort(function (x, y) {
					return x.density > y.density ? -1 : 1;
				});
		...
		function avoidThreats(threats, cell) {
			...
			return {
				id : -5,
				x : cell.x + direction.x * cell.size * 5,
				y : cell.y + direction.y * cell.size * 5
			};
		}
		...
		function setCellColors(cell, myPoints) {
			...
			if (myPoints.length > 0 && !isTeamMode()) {
				...
				//var size_this = Math.min.apply(null, cells.map(function (x) { return x.size; })); // Size of the smallest piece of us
		...
		function drawGrazingLines(ctx) {
			...
			zeach.myPoints.forEach(function (playerBlob) {
				...
				grazeInfo.per_threat.forEach(function (grazeVec) {
					...
					if (!element)
						return; //Wall or dead or something
					...
					drawLine(ctx, playerBlob, {
						x : playerBlob.x + grazeVec.x / maxSize,
						y : playerBlob.y + grazeVec.y / maxSize
					}, "red");
				...
				if (zeach.myPoints.length <= 1) {
					...
					grazeInfo.per_food.forEach(function (grazeVec) {
						...
						drawLine(ctx, element, {
							x : element.x + grazeVec.x / maxSize,
							y : element.y + grazeVec.y / maxSize
						}, "green");
				...
				drawLine(ctx, playerBlob, {
					x : playerBlob.x + cumulatives[1].x / maxSize,
					y : playerBlob.y + cumulatives[1].y / maxSize
				}, "green");
				drawLine(ctx, playerBlob, {
					x : playerBlob.x + cumulatives[0].x / maxSize,
					y : playerBlob.y + cumulatives[0].y / maxSize
				}, "red");

				// Render summary force with special forces, like walls
				ctx.lineWidth = 5;
				drawLine(ctx, playerBlob, {
					x : playerBlob.x + (grazeInfo.fx) / maxSize,
					y : playerBlob.y + (grazeInfo.fy) / maxSize
				}, "orange");
				ctx.lineWidth = 1;
				drawLine(ctx, playerBlob, {
					x : playerBlob.x + 300 * (grazeInfo.fx) / maxSize,
					y : playerBlob.y + 300 * (grazeInfo.fy) / maxSize
				}, "orange");
			});
		...
		function drawTrailTail(ctx) {
			...
			zeach.myPoints.forEach(function (playerBlob) {
				var d = {
					x : playerBlob.nx - playerBlob.x,
					y : playerBlob.ny - playerBlob.y
				};
				drawLine(ctx, playerBlob, {
					x : playerBlob.x - d.x * trailScale,
					y : playerBlob.y - d.y * trailScale
				}, myColor);
		...
		function Wa() {
			...
			C.onmousedown = function (a) {
				/*new*/
				if (isPlayerAlive() && rightClickFires) {
					fireAtVirusNearestToCursor();
				}
				/*new*/
				return;
		...
		function sa() {
			...
			/*new*/
			/*remap*/
			var extras = " " + getScoreBoardExtrasString(J);
			if (0 != J) {
				if (null == ma) {
					ma = new na(24, "#FFFFFF");
				}
				...
				/*new*/
				/*remap*/
				ma.setValue("Score: " + ~~(J / 100) + extras);
				c = ma.G();
				a$$0 = c.width;
				f.globalAlpha = 0.2;
				f.fillStyle = "#000000";
				...
				/*mikey*/
				/*remap*/
				(zeach.myPoints && zeach.myPoints[0] && OnUpdateMass(hb()));
			}
			ib();
			b$$0 = Date.now() - b$$0;
			if (b$$0 > 1E3 / 60) {
			...
			/*new*/
			/*remap*/
			displayDebugText(zeach.ctx, zeach.textFunc); // second param is same as above 'new ??(24,  "#FFFFFF");'
		}
		...
		if (g.location.ancestorOrigins && (g.location.ancestorOrigins.length && "https://apps.facebook.com" != g.location.ancestorOrigins[0])) {
			...
			if ("undefined" == typeof console || ("undefined" == typeof DataView || ("undefined" == typeof WebSocket || (null == Ua || (null == Ua.getContext || null == g.localStorage))))) {
				...
				/*new*///g.connect = La;
				/*new*/
				g.connect2 = g.connect;
				g.connect = La;
				setTimeout(function () {
					try {
						g.connect2("Killing_original_websocket", "");
					} catch (err) {}
				...
				Oa.prototype = {
					...
					T : function (a) {
						if (this.I()) {
							...
							/*new*/
							drawCellInfos.call(this, zeach.isColors, zeach.ctx);
							if (b) {
								a.beginPath();
							...
							//}
							/*new*/
							var c = customSkins(this, zeach.defaultSkins, zeach.imgCache, zeach.isShowSkins, zeach.gameMode);
							...
							/*new*/
							zeach.ctx.globalAlpha = (isSpecialSkin(this.name.toLowerCase()) || _.contains(zeach.myIDs, this.id)/*|| isBitDoSkin(this.name.toLowerCase())*/
							) ? 1 : 0.5;
							if (!(null == e)) {
								if (!c) {
									a.save();
							...
							if (0 != this.id) {
								...
								/*new*/
								/*remap*/
								b = drawCellName.call(this, c, d, e);
				...
				/*new*/
				/*remap*/
				restoreCanvasElementObj(na.prototype);
	...
	function DrawStats(game_over) {
		...
		if (stats.time_of_death !== null) {
			...
			var scale = Math.max.apply(Math, chart_data.map(function (o) {
						return o.y;
					})) / 16;
			var scaled_data = num_cells_data.map(function (a) {
					return {
						x : a.x,
						y : a.y * scale
					};
				});
			stat_chart.options.data.push({
				type : "line",
				dataPoints : scaled_data,
				toolTipContent : " "
			});
			stat_chart.render();
		} else {
	...
	function RenderStats(reset) {
		if (reset)
			g_layout_width = g_display_width;
	...
	function sfx_play(id) {
		if (document.getElementById("sfx").value == 0)
			return;
		var event = ssfxs[id];
	...
	var StartBGM = function () {
		if (document.getElementById("bgm").value == 0)
			return;
		if (bgmusic.src == "") {
	...
	var col1 = $("#col1");
	AppendCheckboxP(col1, 'option3', ' Draw Trailing Tail', window.cobbler.drawTail, function (val) {
		window.cobbler.drawTail = val;
	});
	AppendCheckboxP(col1, 'option4', ' Draw Split Guide', window.cobbler.splitGuide, function (val) {
		window.cobbler.splitGuide = val;
	});
	AppendCheckboxP(col1, 'rainbow-checkbox', ' Rainbow Pellets', window.cobbler.rainbowPellets, function (val) {
		window.cobbler.rainbowPellets = val;
	});
	AppendCheckboxP(col1, 'option7', ' Names under blobs', window.cobbler.namesUnderBlobs, function (val) {
		window.cobbler.namesUnderBlobs = val;
	});
	AppendCheckboxP(col1, 'gridlines-checkbox', ' Show Gridlines', window.cobbler.gridLines, function (val) {
		window.cobbler.gridLines = val;
	});
	...
	var col2 = $("#col2");
	AppendCheckboxP(col2, 'option1', ' Acid Mode', window.cobbler.isAcid, function (val) {
		window.cobbler.isAcid = val;
	});
	AppendCheckboxP(col2, 'litebrite-checkbox', ' Lite Brite Mode', window.cobbler.isLiteBrite, function (val) {
		window.cobbler.isLiteBrite = val;
	});
	...
	$('input[name="DebugLevel"]').change(function () {
		window.cobbler.debugLevel = $(this).val();
	});
	col2.append('<h3>Grazer</h3>');
	AppendCheckboxP(col2, 'autorespawn-checkbox', ' Grazer Auto-Respawns', window.cobbler.autoRespawn, function (val) {
		window.cobbler.autoRespawn = val;
	});
	AppendCheckboxP(col2, 'option5', ' Visualize Grazer', window.cobbler.visualizeGrazing, function (val) {
		window.cobbler.visualizeGrazing = val;
	});
	...
	AppendCheckboxP(col3, 'amConnect-checkbox', ' AgarioMods Connect *skins', window.cobbler.amConnectSkins, function (val) {
		window.cobbler.amConnectSkins = val;
	});
	AppendCheckboxP(col3, 'amExtended-checkbox', ' AgarioMods Extended skins', window.cobbler.amExtendedSkins, function (val) {
		window.cobbler.amExtendedSkins = val;
	});
	AppendCheckboxP(col3, 'imgur-checkbox', ' Imgur.com  i/skins', window.cobbler.imgurSkins, function (val) {
		window.cobbler.imgurSkins = val;
	});
